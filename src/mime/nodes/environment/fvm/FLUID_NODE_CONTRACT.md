...
| `"surface_integral"`  | `u_pre_ibm`, `p`       | Cauchy stress integrated on a cell shell just outside the body (default 1.5–3.5 cells). Cleanest in unconfined regimes; suffers from diffuse-band gradient contamination at λ ≳ 0.15 unless cpr ≥ 6. |
| `"momentum_deficit"`  | `u`, `p`               | Control-volume momentum balance (`F = ΔM + ΔP·A + ρ·f·V_CV − F_wall`). Recommended at moderate-to-high confinement (λ ≳ 0.15). Requires `_pipe_radius` attribute on the node. |
