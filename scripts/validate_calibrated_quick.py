#!/usr/bin/env python3
"""Quick validation of twin-LBM wall correction.

Tests F_x (transverse), F_z (axial), T_z (rotation) at 48³ and 64³.
Uses the twin-LBM approach: walled LBM minus free-space LBM at eval
spheres, IB-BEM body mismatch cancels exactly.

Key metric: does F_x at 64³ improve (or at least not regress) vs 48³?

Previous uncalibrated results:
  48³: F_x=30.60 (3.7%), F_z=46.19 (4.0%), T_z=25.42 (1.0%)
  64³: F_x=29.87 (6.0%), F_z=41.51 (6.5%), T_z=25.42 (1.0%)
"""
...
def main():
    ...
    for N_lbm in resolutions:
        ...
        print(f"=== N={N_lbm}³, twin-LBM ===", flush=True)
        t0 = time.time()
    ...
    # Summary
    print("=" * 70, flush=True)
    print("TWIN-LBM WALL CORRECTION RESULTS", flush=True)
    print("=" * 70, flush=True)
    header = f"{'':>6}"
    for N in resolutions:
        header += f"  {N}³/twin"
    header += f"  {'NN-BEM':>10}  {'old':>8}"
    print(header, flush=True)
    print("-" * 70, flush=True)

    # Old (uncalibrated) results for comparison
    old_errs = {
        0: {48: 3.7, 64: 6.0},
        2: {48: 4.0, 64: 6.5},
        5: {48: 1.0, 64: 1.0},
    ...
    for i, l in enumerate(labels):
        ...
        if i in old_errs:
            old_str = "/".join(f"{old_errs[i].get(N, '?')}%" for N in resolutions)
            row += f"  ({old_str})"
        print(row, flush=True)

    print("-" * 70, flush=True)
    if all_pass:
        print("✓ ALL PASS: twin-LBM < 5% for all directions", flush=True)
    else:
