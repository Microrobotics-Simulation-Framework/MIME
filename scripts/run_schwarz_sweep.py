...
Designed for H100 SXM (80GB VRAM). Uses fixed dx to ensure consistent
interface sphere resolution across all κ values.

Usage:
    python3 scripts/run_schwarz_sweep.py [--resolution 96] [--output results.json]
"""
...
# XLA compilation flags (critical for reasonable compile times)
...
# T2.6b reference (Bouzidi+FSI, 192^3, from umr_training_v2_bouzidi.h5)
T26B_TORQUE_LU = {
    0.15: 87.82,
    0.22: 100.97,
    0.30: 108.54,
...
def generate_umr_mesh(mc_resolution=24):
    ...
    return (
        jnp.array(mesh_mm.points * 1e-3),   # points in meters
        jnp.array(mesh_mm.weights * 1e-6),   # weights in m²
        mesh_mm.n_points,
    )


def compute_unconfined_torque(body_pts, body_wts, eps):
    """Compute free-space rotational drag torque T_z for omega_z = 1."""
...
def run_schwarz_at_kappa(kappa, body_pts, body_wts, N_b, N_lbm,
                          iface_R, n_spinup=1000, n_schwarz_iter=5,
                          n_schwarz_lbm=100):
    """Run Schwarz-coupled BEM+LBM at a single confinement ratio."""
    vessel_R_m = FIN_R_M / kappa
    gap_m = vessel_R_m - iface_R

    # Fixed dx: iface_R maps to ~15 lattice units
    dx = iface_R / 15.0
    ...
    # Check domain fits
    if vessel_R_lu > N_lbm / 2 * 0.95:
        print(f"  WARNING: vessel_R={vessel_R_lu:.1f} lu > {N_lbm/2*0.95:.1f} — "
              f"increase N_lbm or decrease iface_R")
        return None

    print(f"  dx={dx*1e3:.4f}mm, vessel_R={vessel_R_lu:.1f}lu, "
          f"iface_R={iface_R_lu:.1f}lu, gap={gap_lu:.1f}lu, eps={eps:.2e}")
    ...
    iface_lbm = sphere_surface_mesh(
        center=(N_lbm / 2 * dx,) * 3, radius=iface_R, n_refine=2)
    lbm = LBMFarFieldNode(
        "lbm", timestep=dt_lbm,
    ...
    return {
        "kappa": kappa,
        "T_z": T_z,
        "vessel_R_lu": float(vessel_R_lu),
        "iface_R_lu": float(iface_R_lu),
        "gap_lu": float(gap_lu),
        "eps": float(eps),
        "dx": float(dx),
    }
...
def main():
    ...
    parser.add_argument("--kappas", type=str, default="0.15,0.22,0.30,0.35,0.40",
                        help="Comma-separated confinement ratios")
    parser.add_argument("--iface-factor", type=float, default=2.0,
                        help="Interface sphere radius = factor × fin_R")
    args = parser.parse_args()

    kappas = [float(k) for k in args.kappas.split(",")]
    N_lbm = args.resolution
    iface_R = args.iface_factor * FIN_R_M

    print(f"=== VER-029: Schwarz BEM-LBM Drag Multiplier Sweep ===")
    print(f"Device: {jax.devices()[0]}")
    print(f"N_lbm: {N_lbm}^3, iface_R: {iface_R*1e3:.2f}mm")
    ...
    # Sweep
    results = {"unconfined_T_z": T_free, "runs": []}
    t_total = time.time()
    ...
    for kappa in kappas:
        ...
        # Scale spin-up with diffusion time
        vessel_R_m = FIN_R_M / kappa
        dx = iface_R / 15.0
        gap_lu = (vessel_R_m - iface_R) / dx
        nu_lu = (0.8 - 0.5) / 3.0
        diff_time = gap_lu**2 / nu_lu
        n_spinup = max(500, int(diff_time * 3))

        result = run_schwarz_at_kappa(
            kappa, body_pts, body_wts, N_b, N_lbm, iface_R,
            n_spinup=n_spinup,
        )
    ...
    # Normalize to κ=0.15 and compare with T2.6b
    t26b_ref = T26B_TORQUE_LU[0.15]
    runs_by_k = {r["kappa"]: r for r in results["runs"]}

    if 0.15 in runs_by_k:
        ref_T = runs_by_k[0.15]["T_z"]
        print("=== Normalized drag multipliers (vs κ=0.15) ===")
        print(f"{'κ':>6} {'Schwarz':>8} {'T2.6b':>8} {'err%':>6} {'result':>6}")
        for k in sorted(runs_by_k.keys()):
            s = runs_by_k[k]["T_z"] / ref_T
            t = T26B_TORQUE_LU[k] / t26b_ref
            err = abs(s - t) / t * 100
            passed = "PASS" if err < 10 else "FAIL"
            runs_by_k[k]["mult_norm"] = s
            runs_by_k[k]["t26b_mult"] = t
            runs_by_k[k]["error_pct"] = err
            runs_by_k[k]["passed"] = passed
            print(f"{k:>6.2f} {s:>8.3f} {t:>8.3f} {err:>5.1f}% {passed:>6}")
