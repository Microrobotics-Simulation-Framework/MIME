...
def build_graph(design_name="FL-9", vessel_name='1/4"',
                mu_Pa_s=1e-3,
                delta_rho=410.0,
                volume_m3=UMR_VOLUME_M3_FL,
                B_strength_mT=1.2,
                field_freq_hz=10.0,
                dt=5e-4,
                ...
                lubrication_epsilon_mm=0.15):
    """Build the MADDENING simulation graph for a scenario."""
    params = FL_PARAMS[design_name]
    ...
    magnet_node = PermanentMagnetResponseNode(
        ...
        moment_axis=(1.0, 0.0, 0.0),  # moment perpendicular to z-axis (de Jongh)
    )
    ...
    mlp_node = MLPResistanceNode(
        "mlp_drag", dt,
        mlp_weights_path=str(MLP_WEIGHTS),
        nu=params["nu"], L_UMR_mm=params["L_UMR_mm"],
        R_cyl_UMR_mm=R_CYL_UMR_MM,
        R_ves_mm=R_ves_mm,
        mu_Pa_s=mu_Pa_s,
    )

    # Vessel constraint: body center must stay within (R_ves - R_max_body - safety).
    # R_max_body = 2.08 mm, so effective allowed offset = R_ves - 2.08 - 0.1 mm safety.
    R_max_body_mm = R_CYL_UMR_MM * 1.33
    effective_R_mm = max(R_ves_mm - R_max_body_mm - 0.1, 0.1)
    vessel_constraint = CylindricalVesselConstraint(
        radius=effective_R_mm * 1e-3,
        half_length=0.05,  # 100 mm half-length = 200 mm vessel
        axis=2,  # z-axis
    )

    # Effective mass and inertia for inertial integration.
    # Volume ≈ 5.7e-8 m³, effective density 1410 kg/m³ → m ≈ 8e-5 kg.
    # Moment of inertia for a cylinder rotating about its axis: I = m R²/2 ≈ 1e-10 kg·m².
    m_eff = delta_rho * volume_m3 + 1000.0 * volume_m3  # total body mass
    I_eff = 0.5 * m_eff * (R_CYL_UMR_MM * 1e-3)**2

    body_node = RigidBodyNode(
        "body", dt,
        semi_major_axis_m=R_CYL_UMR_MM * 1e-3 * 1.33,
    ...
    # Optional lubrication correction: routes MLP R + body state through a
    # near-wall asymptotic correction, and re-emits drag. The body then
    # reads drag from the lubrication node instead of directly from MLP.
    lub_node = None
    if use_lubrication:
        lub_node = LubricationCorrectionNode(
            "lub", dt,
            R_ves_mm=R_ves_mm,
            R_max_body_mm=R_CYL_UMR_MM * (1.0 + 0.33),
            mu_Pa_s=mu_Pa_s,
            epsilon_mm=lubrication_epsilon_mm,
            a_eff_mm=R_CYL_UMR_MM * (1.0 + 0.33),
        )
    ...
    # Body pose + velocity → MLP (for computing R and drag)
    ...
    # Forces/torques into body (inertial mode integrates F = m a)
    gm.add_edge("gravity", "body", "gravity_force", "external_force", additive=True)
    gm.add_edge("magnet", "body", "magnetic_force", "magnetic_force")
    gm.add_edge("magnet", "body", "magnetic_torque", "magnetic_torque")

    if use_lubrication:
        # MLP R + body state → lubrication → drag → body
    ...
    # External inputs: field control
    gm.add_external_input("field", "frequency_hz", shape=(), dtype=jnp.float32)
    gm.add_external_input("field", "field_strength_mt", shape=(), dtype=jnp.float32)

    return gm, {
        "design": design_name, "vessel": vessel_name,
        "R_ves_mm": R_ves_mm, "dt": dt,
